  ...
  <div class="page-container">
    ...
    <main class="content-wrapper anim-fade-in">
      ...
      <div v-else-if="!post" class="empty-state glass-effect">
        ...
        <NuxtLink :to="`/${lang}/blog`" class="back-link">
      ...
      <article v-else>
        ...
        <!-- Top nav -->
        <div class="top-nav">
          <NuxtLink :to="`/${lang}/blog`" class="back-btn">
            <i class="pi pi-arrow-left"></i>
            Blog
          </NuxtLink>
          <!-- Language switcher → links to alternate slug -->
          <NuxtLink
            v-if="alternateLangUrl"
            :to="alternateLangUrl"
            class="lang-toggle"
          >
        ...
        <!-- Content -->
        <div class="post-content glass-effect anim-slide-up delay-2"
          v-html="pick(post.content, post.content_tr)">
        </div>
        ...
        <NuxtLink :to="`/${lang}/blog`" class="back-footer-link">
          <i class="pi pi-arrow-left"></i>
...
// Alternate language URL (links to the other language's slug)
const alternateLangUrl = computed(() => {
  if (!post.value) return null;
  if (isTr.value) {
    return `/en/blog/${post.value.slug}`;
  }
  return post.value.slug_tr ? `/tr/blog/${post.value.slug_tr}` : null;
});
...
const metaDesc = computed(() =>
  pick(post.value?.meta_description, post.value?.meta_description_tr)
  || pick(post.value?.excerpt, post.value?.excerpt_tr)
  || ''
);
const canonicalUrl = computed(() => `${siteUrl}/${lang.value}/blog/${route.params.slug}`);
const enUrl = computed(() => post.value ? `${siteUrl}/en/blog/${post.value.slug}` : null);
const trUrl = computed(() => post.value?.slug_tr ? `${siteUrl}/tr/blog/${post.value.slug_tr}` : null);
...
useHead(computed(() => ({
  ...
  link: [
    ...
    ...(enUrl.value ? [{ rel: 'alternate', hreflang: 'en', href: enUrl.value }] : []),
    ...(trUrl.value ? [{ rel: 'alternate', hreflang: 'tr', href: trUrl.value }] : []),
    ...(enUrl.value ? [{ rel: 'alternate', hreflang: 'x-default', href: enUrl.value }] : []),
  ],
  script: post.value ? [{
    ...
    children: JSON.stringify({
      ...
      url: enUrl.value || canonicalUrl.value,
      ...(post.value.cover_image && { image: post.value.cover_image }),
    }),
  }] : [],
})));
</script>
...
.page-container {
  ...
  align-items: flex-start; position: relative; overflow: hidden; padding: 2rem;
}
...
.loading-state { display: flex; align-items: center; justify-content: center; padding: 6rem; color: var(--text-muted); font-size: 2rem; }
...
.back-link { display: inline-flex; align-items: center; gap: 0.4rem; color: #818cf8; font-size: 0.9rem; text-decoration: none; }
...
.lang-toggle:hover { background: rgba(129,140,248,0.15); border-color: rgba(129,140,248,0.4); color: #818cf8; }

.post-header { padding: 2rem; display: flex; flex-direction: column; gap: 1rem; }
.post-meta { display: flex; align-items: center; gap: 1.25rem; flex-wrap: wrap; }
.date-text, .author-text { display: flex; align-items: center; gap: 0.35rem; font-size: 0.82rem; color: var(--text-muted); }
.post-title { margin: 0; font-size: 1.75rem; font-weight: 700; line-height: 1.35; color: var(--text-main); }
.post-excerpt { margin: 0; font-size: 1rem; line-height: 1.7; color: var(--text-muted); border-left: 3px solid rgba(129,140,248,0.5); padding-left: 1rem; }
.tags-inline { display: flex; flex-wrap: wrap; gap: 0.4rem; }
.tag-badge { padding: 0.2rem 0.65rem; border-radius: 50px; background: rgba(129,140,248,0.1); border: 1px solid rgba(129,140,248,0.2); color: #818cf8; font-size: 0.75rem; }

.post-content { padding: 2rem 2.5rem; line-height: 1.8; font-size: 1rem; }
:deep(.post-content h2) { font-size: 1.3rem; font-weight: 600; margin: 2rem 0 0.75rem; color: #c4b5fd; }
:deep(.post-content p) { margin: 0 0 1.25rem; color: rgba(248,250,252,0.85); }
:deep(.post-content ul), :deep(.post-content ol) { margin: 0 0 1.25rem; padding-left: 1.5rem; }
:deep(.post-content li) { margin-bottom: 0.5rem; color: rgba(248,250,252,0.85); }
:deep(.post-content strong) { color: var(--text-main); font-weight: 600; }
:deep(.post-content em) { color: #c084fc; font-style: italic; }
:deep(.post-content a) { color: #818cf8; text-decoration: underline; text-underline-offset: 3px; transition: color 0.2s; }
:deep(.post-content a:hover) { color: #c4b5fd; }

.cta-box { display: flex; align-items: center; gap: 1.25rem; padding: 1.5rem 2rem; flex-wrap: wrap; }
.cta-icon { font-size: 2rem; color: #818cf8; flex-shrink: 0; }
.cta-text { display: flex; flex-direction: column; gap: 0.2rem; flex: 1; }
.cta-text strong { font-size: 1rem; color: var(--text-main); }
.cta-text span { font-size: 0.88rem; color: var(--text-muted); }
.cta-btn { padding: 0.65rem 1.5rem; border-radius: 50px; background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; font-weight: 600; font-size: 0.9rem; text-decoration: none; white-space: nowrap; transition: opacity 0.2s, transform 0.2s; }
.cta-btn:hover { opacity: 0.9; transform: translateY(-1px); }

.back-footer-link { display: inline-flex; align-items: center; gap: 0.5rem; color: var(--text-muted); font-size: 0.9rem; text-decoration: none; transition: color 0.2s; }
